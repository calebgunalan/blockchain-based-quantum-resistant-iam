...
export default function SessionManagement() {
  ...
  const fetchStats = async () => {
    try {
      ...
      // Count admin sessions by checking user_roles for each active session
      ...
      if (adminSessions) {
        for (const session of adminSessions) {
          ...
          if (checkedUsers.has(session.user_id)) {
            adminCount++;
            continue;
          }
      ...
      // Ensure current admin session is counted
      if (currentUser) {
        ...
        if (currentRole?.role === 'admin' && !checkedUsers.has(currentUser.id)) {
          adminCount++;
        }
      }
